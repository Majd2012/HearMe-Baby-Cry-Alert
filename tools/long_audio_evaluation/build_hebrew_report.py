...
def add_synthetic_night_section(doc):
    add_heading(doc, "יצירת הקלטת לילה סינתטית", 1)
    add_p(doc, "השתמשנו בהקלטות לילה סינתטיות משום שלא היה בידינו קובץ לילה אמיתי מלא שבו זמני התחלה וסיום של כל אירוע בכי אומתו במדויק. הקלטה סינתטית מאפשרת בדיקה מבוקרת, מכיוון שזמן ההתחלה וזמן הסיום האמיתיים של כל אירוע בכי ידועים מראש.")
    ...
    add_p(doc, "תהליך הבנייה של ההקלטה הסינתטית בוצע כך:")
    add_bullets(doc, [
    ...
    add_p(doc, "זרימת העבודה:", bold=True)
...
def add_alert_logic(doc):
    add_heading(doc, "5. שיפור לוגיקת ההתרעה", 1)
...
def add_config_sections(doc):
    add_heading(doc, "6. תצורת המערכת המקורית", 1)
    ...
    add_heading(doc, "7. התצורה המומלצת הסופית", 1)
...
def add_results_sections(doc):
    add_heading(doc, "8. תוצאות סופיות ועקרון האופטימיזציה", 1)
...
def add_figures(doc):
    add_heading(doc, "9. תרשימים והסברים", 1)
    figures = [
